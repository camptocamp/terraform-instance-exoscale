...
# Code to test
#
resource "exoscale_security_group" "test_basic_instance_centos" {
  name = "test-basic-instance-centos"
}

resource "exoscale_security_group_rules" "test_basic_instance_centos" {
  security_group_id = exoscale_security_group.test_basic_instance_centos.id
...
module "instance" {
  ...
  tags = {
    Name = "testing terraform instance centos"
  }
